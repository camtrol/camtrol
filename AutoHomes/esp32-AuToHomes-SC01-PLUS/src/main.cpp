...
#define LED2_NAME_EEPROM 5  // Unsigned Long
#define LED3_NAME_EEPROM 15 // Unsigned Long
#define LED4_NAME_EEPROM 25 // Unsigned Long
// #define LED5_NAME_EEPROM 50     // Unsigned Long
#define LED6_NAME_EEPROM 35 // Unsigned Long
#define LED7_NAME_EEPROM 45 // Unsigned Long
#define LED8_NAME_EEPROM 55 // Unsigned Long
...
String ledNameOrigin[9] = {" ",        "전체등", "실내등", "사이드", "팝업등",
                           "시트열선", "무드등", "후면등", "어닝등"};
String ledName[9] = {};
...
char ssidAp[30] = "오토홈스 V6.0-CB-005";
char passwordAp[30] = "16240522005";
...
void handleRequest() {
  ...
  s = s + "{" + "\"aux_v\":" + aux_v + ", " + "\"Temperature\":" + temperature +
      ...
      ledFlag[7] + ", " + "\"ledFlag8\":" + ledFlag[8] + ", " + "\"pump\":" +
      pumpFlag + ", " + "\"invertor\":" + invertorFlag + ", " + "\"tv\":" +
      tvFlag + ", " + "\"frige\":" + frigeFlag + ", " + "\"ledAllFlag\":" +
      ledAllFlag + ", " + "\"sleepFlag\":" + sleepFlag + ", " + "\"remote\":" +
      remoteFlag + ", " + "\"popUp\":" + popUpFlag + ", " + "\"popStop\":" +
      popStopFlag + ", " + "\"popDown\":" + popDownFlag + "}";
...
void powerReconnection(lv_event_t *e) {
  Serial.println("Power Reconnection...");
  lv_obj_clear_state(ui_Button1, LV_STATE_CHECKED);
  tft.setBrightness(255);
  wifiStart();
}
...
void brightSlide(lv_event_t *e) { // 화면 밝기 조절...
  int bright = lv_slider_get_value(ui_Slider3);
...
void ledAllProcess() {
  if (!ledFlag[2] && !ledFlag[3] && !ledFlag[4] && !ledFlag[6] && !ledFlag[7] &&
      !ledFlag[8]) {
    ledAllFlag = false;
    lv_obj_clear_state(ui_Button1, LV_STATE_CHECKED);
  } else if (ledFlag[2] && ledFlag[3] && ledFlag[4] && ledFlag[6] &&
             ledFlag[7] && ledFlag[8]) {
    ledAllFlag = true;
    lv_obj_add_state(ui_Button1, LV_STATE_CHECKED);
  }
...
void ledButton1(lv_event_t *e) {
  ...
  if (!ledAllFlag) {
    ...
    lv_slider_set_value(ui_Slider1, s.toInt(), LV_ANIM_OFF);
  }
  s = "전체등";
  // s.toCharArray(label_text, sizeof(label_text));
  s.toCharArray(label_text, s.length() + 1);
...
void ledButton9(lv_event_t *e) { // 물펌프 버튼
  // ledSelect = 1;
  delay(10);
...
void wifiStart() { // WiFi Mode Start...
  ...
  if (networkFlag == AP_MODE) {
    ...
    // char hostName[30]; //= "CamTrol-SC01";
    // byte mac[6];
    // WiFi.macAddress(mac);
    // sprintf(hostName, "CamTrol_%02X:%02X:%02X", mac[3], mac[4], mac[5]);
    // Serial.println(hostName);
    // Set your Static IP address
    // IPAddress local_IP(192, 168, 98, 184);
    // IPAddress local_IP(192, 168, 1, 184);
    // IPAddress local_IP(192, 168, 98, 100);
    // IPAddress local_IP(192, 168, 4, 1);
    // Set your Gateway IP address
    // IPAddress gateway(192, 168, 98, 1);
    // IPAddress gateway(192, 168, 1, 1);
    // IPAddress gateway(192, 168, 98, 176);
    // IPAddress gateway(192, 168, 4, 1);

    // IPAddress subnet(255, 255, 255, 0);
    // IPAddress primaryDNS(192, 168, 98, 176); // optional
    ...
    // s = (WiFi.status() == WL_CONNECTED) ? "연결 되었습니다..."
    //                                     : "핫스팟 네트워크 확인...";
...
void setup() {
  ...
  // Wire.begin(21, 14); // Input Board 통신용...
  // I2Ctwo.begin(SDA_1, SCL_1, 100000);
  // Wire1.begin(12, 11, 400000); // Input Board 통신용...
  I2Ctwo.begin(12, 11); // Input Board 통신용...

  int ahtStatus = 0;

  // for (int i = 0; i < 3; i++) {
  //   if (myAHT10.begin(&Wire1, 0, 0x38) != true) {
  //     Serial.println(F("AHT10 Not Found"));
  //     delay(50);
  //   } else {
  //     ahtFlag = true;
  ...
  // while (myAHT10.begin(&Wire1, 0, 0x38) != true) {
  ...
  // rtc.adjust(DateTime(F("Mar 16 2024"), F("18:57:10")));
  ...
  // Serial.print("Sleep Time EEPROM Value : ");
  // Serial.println(EEPROM.readByte(SLEEP_COUNT_EEPROM));
...
void loop() {
  ...
  if (loopCount > 180) { // 약 1초 마다 실실행...
    ...
    if (tempCount > 3) {
      ...
      tempToggleFlag = tempToggleFlag ? false : true;
      // tempToggleFlag ~ = tempToggleFlag;
      if (tempToggleFlag) {
        lv_obj_add_flag(ui_ImageTemp, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(ui_LabelTemp, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(ui_ImageHumi, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(ui_LabelHumi, LV_OBJ_FLAG_HIDDEN);
      } else {
        lv_obj_clear_flag(ui_ImageTemp, LV_OBJ_FLAG_HIDDEN);
        lv_obj_clear_flag(ui_LabelTemp, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(ui_ImageHumi, LV_OBJ_FLAG_HIDDEN);
        lv_obj_add_flag(ui_LabelHumi, LV_OBJ_FLAG_HIDDEN);
      }
      tempCount = 0;
    }
    tempCount++;
