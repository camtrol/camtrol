...
#define TV 40
#define FRIGE 39
// #define POP_UP_FLAG P3
#define POP_UP 35
#define POP_DOWN 45
// #define POP_STOP P6
// #define POP_DN_LMT P7
...
#define AUTO_ON_PUMP_EEPROM 9
#define AUTO_ON_TV_EEPROM 10
#define AUTO_ON_FRIGE_EEPROM 11
#define AUTO_ON_INV_EEPROM 12
...
char ssidAp[30] = "오토홈스 V6.0-B-011";
char passwordAp[30] = "11240403011";
char msg[100];
...
// PCF8574 pcf8574(&I2Ctwo, 0x20, 42, 2, INTERRUPTED_PIN, keyPressedOnPCF8574);
...
void serialEvent2() {
  ...
  if (chanel == "LED1") {
    ...
    if (value) {
      ...
      popStopFlag = false;
      popDownFlag = false;
      // pcf8574.digitalWrite(POP_DOWN, LOW);
      // delay(50);
      digitalWrite(POP_UP, HIGH);
    } else {
      popUpFlag = false;
      // pcf8574.digitalWrite(POP_UP, LOW);
    }
    ...
    // pcf8574.digitalWrite(POP_STOP, HIGH);
    if (value) {
      popUpFlag = false;
      popStopFlag = true;
      popDownFlag = false;
    ...
    // delay(500); // 정지신호 유지 시간
    // pcf8574.digitalWrite(POP_STOP, LOW);
    ...
    if (value) {
      ...
      ledcWrite(ledPwmCh[6], 0);
      ledcWrite(ledPwmCh[7], 0);
      ledcWrite(ledPwmCh[8], 0);
...
void btnPowerOnExeSave(lv_event_t *e) {
  ...
  if (lv_obj_get_state(ui_onSwitch9) == LV_STATE_CHECKED)
    EEPROM.writeBool(AUTO_ON_PUMP_EEPROM, true);
  else
    EEPROM.writeBool(AUTO_ON_PUMP_EEPROM, false);
...
// void keyPressedOnPCF8574() { keyPressed = true; }
...
void handleRequest() {
  ...
  s = s + "{" + "\"aux_v\":" + aux_v + ", " + "\"Temperature\":" + temperature +
      ...
      ", " + "\"ledFlag8\":" + ledFlag[8] + ", " + "\"pump\":" + pumpFlag +
...
void setup() {
  ...
  // pcf8574.pinMode(INVERTOR, OUTPUT);
  // pcf8574.pinMode(TV, OUTPUT);
  // pcf8574.pinMode(FRIGE, OUTPUT);
  // pcf8574.pinMode(POP_UP_FLAG, INPUT);
  // pcf8574.pinMode(POP_UP, OUTPUT);
  // pcf8574.pinMode(POP_DOWN, OUTPUT);
  // pcf8574.pinMode(POP_STOP, OUTPUT);
  // pcf8574.pinMode(POP_DN_LMT, INPUT);
  ...
  // pcf8574.digitalWrite(INVERTOR, LOW);
  // pcf8574.digitalWrite(TV, LOW);
  // pcf8574.digitalWrite(FRIGE, LOW);
  // pcf8574.digitalWrite(POP_UP, LOW);
  // pcf8574.digitalWrite(POP_DOWN, LOW);
  // pcf8574.digitalWrite(POP_STOP, LOW);
  ...
  String LVGL_Arduino = "오토홈스 콘트롤러! ";
  LVGL_Arduino += String('V') + lv_version_major() + "." + lv_version_minor() +
                  "." + lv_version_patch() + '\n';

  Serial.println(LVGL_Arduino);
  Serial.println("I am LVGL_Arduino");
  lv_print("오토홈스 콘트롤러 시작...");
  ...
  if (EEPROM.readBool(AUTO_ON_1_EEPROM)) { // 전체등..
  ...
  if (EEPROM.readBool(AUTO_ON_TV_EEPROM)) { // TV..
    lv_obj_add_state(ui_onSwitch10, LV_STATE_CHECKED);
    tvFlag = true;
    digitalWrite(TV, HIGH);
  }

  if (EEPROM.readBool(AUTO_ON_FRIGE_EEPROM)) { // 냉장고..
    lv_obj_add_state(ui_onSwitch11, LV_STATE_CHECKED);
    frigeFlag = true;
    digitalWrite(FRIGE, HIGH);
  }

  if (EEPROM.readBool(AUTO_ON_INV_EEPROM)) { // 인버터..
