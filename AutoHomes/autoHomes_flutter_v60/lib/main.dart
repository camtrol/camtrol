            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: Text(
                    logoFlag ? logoText : 'CamTrol V6.0',
                    style: TextStyle(
                      fontWeight: FontWeight.bold,
                      fontStyle: FontStyle.italic,
                      fontSize: mobile
                ...
                onTap: () async {
                  ...
                  if (ledCh == 1) {
                    V.ledStatus[ledCh]
                        ? allLedStatus = true
                        : allLedStatus = false;
                    server.getReq("ALL", allLedStatus ? 1 : 0);
                  } else {
                    server.getReq("LED", ledCh);
                  }
...
class SeatHeatProcess extends StatefulWidget {
  const SeatHeatProcess({Key? key}) : super(key: key);

  @override
  State<SeatHeatProcess> createState() => _SeatHeatProcessState();
}

class _SeatHeatProcessState extends State<SeatHeatProcess> {
  @override
  Widget build(BuildContext context) {
    return InkWell(
      child: Container(
      ...
      onTap: () async {
        ...
        if (V.ledStatus[5] == true) {
          ...
          // tvStatus = true;
          V.ledStatus[5] = true;
          V.monitor = 'Seat Heat On..';
        }
        setState(() {});
        // server.getReq("TV", 0);
        server.getReq("LED", 5);
        V.ledStatus[5] ? print('Seat Heat On..') : print('Seat Heat Off..');
      },
    );
  }
}
    ...
    return InkWell(
      ...
      onTap: () async {
        if (pumpStatus == true) {
          ...
          pumpStatus = true;
          await server.getReq("PUMP", 1);
          V.monitor = '물펌프 On..';
        }
        setState(() {});
        pumpStatus ? print('물펌프 On..') : print('물펌프 Off..');
      },
    );
  }
}
