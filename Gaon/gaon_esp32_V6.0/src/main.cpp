...
#define HEATER 47 // PCF8574 무시동 히터...
// #define CLEAN_VALVE 1  // PCF8574 CLEAN VALVE...
// #define WASTE_VALVE 2  // PCF8574 WASTE VALVE...
...
// PCF8574 pcf8574(&I2Ctwo, 0x20, 42, 2);
...
// JsonObject liftRoot;
String payload;
String EEPROM_SSID_FACTORY = ""; // EEPROM 주소 0부터 SSID를 읽기
String EEPROM_PSWD_FACTORY = ""; // EEPROM 주소 20부터 저장된 비밀번호 읽기
...
typedef struct struct_message {
  ...
  int ledValue[7] = {0, 100, 100, 100, 100, 100, 100};
  int sp3Value = 100;
  int sp4Value = 100;
...
void voltSave(lv_event_t *e) {
  EEPROM.writeFloat(90, voltAdjust);
  EEPROM.commit();
}
...
void keyPressedOn() {
  ...
  // bool _outFlag[2];
  int pwmCount;
  m.ledAllFlag ? m.ledAllFlag = false : m.ledAllFlag = true;
  // LED Power On Exe 처리...
  // _ledFlag[1] = EEPROM.readBool(70);
  // _ledFlag[2] = EEPROM.readBool(71);
  // _ledFlag[3] = EEPROM.readBool(72);
  // _ledFlag[4] = EEPROM.readBool(73);
  // _ledFlag[5] = EEPROM.readBool(74);
  // _ledFlag[6] = EEPROM.readBool(75);

  // OUT Power On Exe 처리...
  eepOutFlag[1] = EEPROM.readBool(80);
  // keyPressed = true;

  // Serial.print("ledAllFlag: ");
  // Serial.println(m.ledAllFlag);
  // Serial.print("_ledFlag: ");
  // Serial.print(_ledFlag[1]);
  // Serial.print(", ");
  // Serial.print(_ledFlag[2]);
  // Serial.print(", ");
  // Serial.print(_ledFlag[3]);
  // Serial.print(", ");
  // Serial.print(_ledFlag[4]);
  // Serial.print(", ");
  // Serial.print(_ledFlag[5]);
  // Serial.print(", ");
  // Serial.print(_ledFlag[6]);
  // Serial.print(", _outFlag: ");
  // Serial.print(_outFlag[1]);
  // Serial.print(", sleepFlag: ");
  // Serial.println(m.sleepFlag);

  if (m.ledAllFlag) { // All LED On...
    lv_print("전체등 켜짐...");
    pwmCount = 0;
    m.sleepFlag = false;
    if (eepOutFlag[1])
      digitalWrite(outCh[1], HIGH);

    while (pwmCount < 4095) {
      for (int i = 1; i < 7; i++) {
        if ((pwmPush[i] < pwmCount) && eepLedFlag[i])
...
void handleJson() {
  ...
  if (division == "LED") {
    ...
    // if (division == "LED") {
    //   if (pwmPush[chanel] <= pwm) { // 현재 PWM 값이 기존 보다 크면 Fast
    //   ON...
    //     while (pwmPush[chanel] < pwm) {
    //       ledcWrite(ledCh[chanel], pwmPush[chanel]);
    //       pwmPush[chanel] += 5;
    //       delay(30);
    //     }
    //     pwmPush[chanel] = pwm;
    //     ledcWrite(ledCh[chanel], pwmPush[chanel]);
    //   } else { // 현재 PWM 값이 기존 보다 작으면 Slow OFF...
    //     while (pwmPush[chanel] > pwm) {
    //       ledcWrite(ledCh[chanel], pwmPush[chanel]);
    //       pwmPush[chanel] -= 5;
    //       delay(30);
    ...
    if (value) {
      ...
      if (chanel == 2) { // 과 돌입 전류 예방...
        ...
        if (m.outOnOff[chanel] != value) {
          // pcf8574.digitalWrite(HEATER, ON);
          digitalWrite(HEATER, ON);
          delay(600);
          // pcf8574.digitalWrite(HEATER, OFF);
    ...
    if (value == 1) {
      m.invertorFlag = true;
      // pcf8574.digitalWrite(INVERTOR, ON);
      digitalWrite(INVERTOR, ON);
    } else {
      m.invertorFlag = false;
      // pcf8574.digitalWrite(INVERTOR, OFF);
  ...
  // }
    ...
    if (value == 1) {
      m.clnValveFlag = true;
      // pcf8574.digitalWrite(CLEAN_VALVE, ON);
      digitalWrite(CLEAN_VALVE, ON);
    } else {
      m.clnValveFlag = false;
      // pcf8574.digitalWrite(CLEAN_VALVE, OFF);
    ...
    if (value == 1) {
      m.wstValveFlag = true;
      // pcf8574.digitalWrite(WASTE_VALVE, ON);
      digitalWrite(WASTE_VALVE, ON);
    } else {
      m.wstValveFlag = false;
      // pcf8574.digitalWrite(WASTE_VALVE, OFF);
...
void setup() {
  ...
  // ledcSetup(ledCh[1], freq, resolution);
  // ledcSetup(ledCh[2], freq, resolution);
  // ledcSetup(ledCh[3], freq, resolution);
  // ledcSetup(ledCh[4], freq, resolution);
  // ledcSetup(ledCh[5], freq, resolution);
  // ledcSetup(ledCh[6], freq, resolution);

  ledcAttachPin(HT_FAN, motorCh[1]);
  ledcAttachPin(HT_MOTOR_1, motorCh[2]);
  ledcAttachPin(HT_MOTOR_2, motorCh[3]);

  // ledcAttachPin(LED01, ledCh[1]);
  // ledcAttachPin(LED02, ledCh[2]);
  // ledcAttachPin(LED03, ledCh[3]);
  // ledcAttachPin(LED04, ledCh[4]);
  // ledcAttachPin(LED05, ledCh[5]);
  // ledcAttachPin(LED06, ledCh[6]);
  ...
  I2Ctwo.begin(42, 2);
  Serial.begin(115200);
  // lift.begin(19200, SERIAL_8N1, RXD2, TXD2);
  lift.begin(19200, SERIAL_8N1, 4, 6);

  digitalWrite(ENABLE, HIGH); // RS Send

  // pcf8574.pinMode(HEATER, OUTPUT, OFF);
  // pcf8574.pinMode(INVERTOR, OUTPUT, OFF);
  // pcf8574.pinMode(CLEAN_VALVE, OUTPUT, OFF);
  // pcf8574.pinMode(WASTE_VALVE, OUTPUT, OFF);
  // pcf8574.pinMode(LIFT_FLAG, INPUT);
  // pcf8574.pinMode(LIFT_POWER, OUTPUT, OFF);
  // pcf8574.pinMode(LIGHT_SWITCH, INPUT_PULLUP);
  ...
  int ahtStatus = 0;
  // while (pcf8574.begin() != true) {
  //   Serial.println(F("Could not find PCF8574 Check wiring"));
  //   lv_print("확장 I/O 회로 점검 요망...");
  //   delay(100);
  //   if (ahtStatus > 5)
  //     break;
  //   ahtStatus++;
  ...
  voltAdjust = EEPROM.readFloat(90);
  if (isnan(voltAdjust)) {
    voltAdjust = 0.0;
    EEPROM.writeFloat(90, 0.0);
    EEPROM.commit();
  }
  ...
  s = lv_label_get_text(ui_SSIDLabel);
...
void loop() {
  ...
  // if (!pcf8574.digitalRead(LIGHT_SWITCH)) {
